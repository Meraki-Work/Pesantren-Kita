    ...
    <main class="flex-1 p-4 overflow-y-auto">
        <div class="p-6 box-bg">
            ...
            <div class="container mx-auto px-4 py-8">
                <form action="{{ route('notulen.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-6">
                        {{-- Agenda Rapat --}}
                        <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200" required>
                                @error('tanggal')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        {{-- Peserta --}}
                        <div id="peserta-container" class="space-y-3">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Peserta Rapat <span class="text-red-500">*</span>
                            </label>

                            @php
                            $pesertaValues = old('peserta', ['', '']);
                            @endphp

                            @foreach($pesertaValues as $index => $value)
                            <div class="flex gap-2 items-center peserta-row">
                                <div class="flex-1">
                                    <input type="text" name="peserta[]" value="{{ $value }}"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                                        placeholder="Nama peserta" {{ $index === 0 ? 'required' : '' }}>
                                </div>
                                @if($index === 0)
                                ...
                                @else
                                <button type="button" onclick="removePeserta(this)"
                    ...
                    <div class="space-y-6">
                        ...
                        <div id="poin-container" class="space-y-3">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Poin-Poin Rapat (Hasil) <span class="text-red-500">*</span>
                            </label>

                            @php
                            $hasilValues = old('hasil', ['', '']);
                            @endphp

                            @foreach($hasilValues as $index => $value)
                            <div class="flex gap-2 items-center poin-row">
                                <div class="flex-1">
                                    <input type="text" name="hasil[]" value="{{ $value }}"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                                        placeholder="Tambah poin hasil rapat" {{ $index === 0 ? 'required' : '' }}>
                                </div>
                                @if($index === 0)
                        ...
                        <div class="pt-6 border-t border-gray-200">
                            <div class="flex flex-col sm:flex-row gap-3 justify-end">
                                <a href="{{ route('notulen.index') }}"
                                    class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition duration-200 font-medium text-center">
                                    Batal
                                </a>
                                <button type="submit"
