                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="hidden" name="ponpes_id" value="{{ old('ponpes_id', $landingContent->ponpes_id) }}">
                                <div class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-700">
                                    {{ $landingContent->ponpes ? $landingContent->ponpes->nama_ponpes : ($landingContent->ponpes_id ?? 'Pesantren Anda') }}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <!-- Locked content type for edit: use hidden input for submission and disabled select for display -->
                                <input type="hidden" name="content_type" value="{{ old('content_type', $landingContent->content_type) }}">
                                <input type="hidden" name="content_type"
                                    value="{{ old('content_type', $landingContent->content_type) }}">
                                <div class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-700">
                                    {{ $contentTypes[$landingContent->content_type] ?? '-' }}
                                </div>
                            </div>
                        </div>

                        <!-- Judul dan Posisi (render khusus per tipe konten) -->
                        @php
    $currentType = old('content_type', $landingContent->content_type);
    // Normalize legacy spelling
    if ($currentType === 'about_mision') {
        $currentType = 'about_mission';
    }
    $showPosition = in_array($currentType, ['about_founder', 'about_leader']);
                        @endphp

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="{{ $showPosition ? '' : 'md:col-span-2' }}">
