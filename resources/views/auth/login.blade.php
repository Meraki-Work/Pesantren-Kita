...
<body class="min-h-screen flex">
  ...
  <div class="w-full md:w-1/2 flex items-center justify-center bg-[#344E41]">
    <div class="bg-white bg-opacity-90 p-6 rounded-xl shadow-md max-w-md w-[400px] h-[450px] flex flex-col items-center justify-center">
      ...
      <form id="loginForm" action="{{ route('login.post') }}" method="POST" class="space-y-4 flex flex-col items-center w-full">
        @csrf
        ...
        <button id="btnLogin" type="submit"
          class="w-full max-w-[300px] flex justify-center items-center bg-green-500 hover:bg-green-600 text-white font-semibold py-2 rounded-lg transition duration-300 text-sm">
  ...
  <script>
    document.getElementById('loginForm').addEventListener('submit', function() {
      ...
      document.getElementById('btnLogin').classList.add('bg-green-400');
